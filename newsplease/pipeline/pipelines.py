...
class RedisStorageClient(StrictRedis):
    ...
    * It is possible to set a TTL on objects stored
    ...
    @classmethod
    def _get_name(
        cls, collection: Collections, url: str, version: Optional[str] = None
    ) -> str:
    ...
    def purge(self):
        ...
        if self.dangerously_flush_db:
            ...
            full_scan = chain.from_iterable(
                (
                    self.scan_iter(
                        match=f"{Collections.CurrentVersions}{self.separator}*"
                    ),
                    self.scan_iter(
                        match=f"{Collections.ArchiveVersions}{self.separator}*"
                    ),
                ),
            )
...
class RedisStorage(ExtractedInformationStorage):
    ...
    def __init__(self):
        ...
        self.enable_archive = self.cfg.parser.getboolean(
            "Redis", "enable_archive", fallback=True
        )
    ...
    def process_item(self, item: Any, spider: scrapy.Spider):
        ...
        # Add the new version of the article to the CurrentVersion table
        new_version = cast(
            Dict[str, Any], ExtractedInformationStorage.extract_relevant_info(item)
        )
